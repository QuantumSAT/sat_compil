 ...
 */
#ifndef _SIS_ESPRESSO_H_
#define _SIS_ESPRESSO_H_

#include <stdio.h>
#include "syn/utility/sparse.h"
#include "syn/twolevel/mincov.h"
...
/* Define host machine characteristics of "unsigned int" */
  ...
  typedef int (*cmp_fn0)(const void*);
  typedef int (*ESP_PFI)(unsigned int**, unsigned int**);
  typedef int (*ESP_PFI2)(unsigned int**, int, int);
  typedef int (*ESP_PFI1)(unsigned int**, int);
...
/* Flag definitions are ... */
  const int PRIME=0x8000;          /* cube is prime */
  const int NONESSEN=0x4000;          /* cube cannot be essential prime */
  const int ACTIVE=0x2000;          /* cube is still active */
  const int REDUND=0x1000;          /* cube is redundant(at this point) */
  const int COVERED=0x0800;          /* cube has been covered */
  const int RELESSEN=0x0400;          /* cube is relatively essential */
...
#define print_bool(x) ((x) == 0 ? "FALSE" : ((x) == 1 ? "TRUE" : "MAYBE"))
  ...
  typedef struct cost_struct {
    ...
    int support;    /* size of support set */
...
#define INOUT
  ...
  const int D_type=2;
  const int R_type=4;
  const int PLEASURE_type=8;               /* output format */
  const int EQNTOTT_type=16;              /* output format algebraic eqns */
  const int KISS_type=128;		/* output format kiss */
  const int CONSTRAINTS_type=256;	/* output the constraints (numeric) */
  const int SYMBOLIC_CONSTRAINTS_type=512;	/* output the constraints (symbolic) */
#if 1
...
#else
  const int FD_TYPE=(F_type | D_type);
  const int FR_type=(F_type | R_type);
  const int DR_type=(D_type | R_type);
  const int FDR_type=(F_type | D_type | R_type);
#endif

  /* Definitions for the debug variable */
  const int COMPL=0x0001;
  const int ESSEN=0x0002;
  const int EXPAND=0x0004;
  const int EXPAND1=0x0008;
  const int GASP=0x0010;
  const int IRRED=0x0020;
  const int REDUCE=0x0040;
  const int REDUCE1=0x0080;
  const int SPARSE=0x0100;
  const int TAUT=0x0200;
  const int EXACT=0x0400;
  const int MINCOV=0x0800;
  const int MINCOV1=0x1000;
  const int SHARP=0x2000;
  const int IRRED1=0x4000;
...
#define ESPRESSO_VERSION					\
  ...
  const int COMPL_TIME=1;
  const int ONSET_TIME=2;
  const int ESSEN_TIME=3;
  const int EXPAND_TIME=4;
  const int IRRED_TIME=5;
  const int REDUCE_TIME=6;
  const int GEXPAND_TIME=7;
  const int GIRRED_TIME=8;
  const int GREDUCE_TIME=9;
  const int PRIMES_TIME=10;
  const int MINCOV_TIME=11;
  const int MV_REDUCE_TIME=12;
  const int RAISE_IN_TIME=13;
  const int VERIFY_TIME=14;
  const int WRITE_TIME=15;
...
#define EXECUTE(fct,i,S,cost)			\
   ...
   */
  extern int esp_cpu_signal;	    	/* if negative, potential runtime problem, should quit espresso */
  extern unsigned int debug;            /* debug parameter */
  extern esp_bool verbose_debug;            /* -v:  whether to print a lot */
  ...
  extern esp_bool echo_comments;		/* turned off by -eat option */
  extern esp_bool echo_unknown_commands;	/* always true ?? */
  extern esp_bool force_irredundant;        /* -nirr command line option */
  extern esp_bool skip_make_sparse;
  extern esp_bool kiss;                     /* -kiss command line option */
  extern esp_bool pos;                      /* -pos command line option */
  extern esp_bool print_solution;           /* -x command line option */
  extern esp_bool recompute_onset;          /* -onset command line option */
  extern esp_bool remove_essential;         /* -ness command line option */
  extern esp_bool single_expand;            /* -fast command line option */
  extern esp_bool summary;                  /* -s command line option */
  extern esp_bool trace;                    /* -t command line option */
  extern esp_bool unwrap_onset;             /* -nunwrap command line option */
  extern esp_bool use_random_order;		/* -random command line option */
  extern esp_bool use_super_gasp;		/* -strong command line option */
  extern char *filename;		/* filename PLA was read from */
  extern esp_bool debug_exact_minimization; /* dumps info for -do exact */
  ...
  struct cdata_struct {
    ...
    esp_bool *is_unate;             /* indicates given var is unate */
...
#ifdef lint
  const int DISJOINT=0x5555;
#else
  const int DISJOINT=0x55555555;
#endif
  
  /* function declarations */
  
  /* cofactor.c */	EXTERN int binate_split_select ARGS((pcube *, pcube, pcube, int));
  ...
  /* contain.c */	EXTERN int d1_rm_equal ARGS((pset *, cmp_fn/*ESP_PFI*/));
  /* contain.c */	EXTERN int rm2_contain ARGS((pset *, pset *));
  /* contain.c */	EXTERN int rm2_equal ARGS((pset *, pset *, pset *, cmp_fn/*ESP_PFI*/));
  /* contain.c */	EXTERN int rm_contain ARGS((pset *));
  /* contain.c */	EXTERN int rm_equal ARGS((pset *, cmp_fn/*ESP_PFI*/));
  /* contain.c */	EXTERN int rm_rev_contain ARGS((pset *));
  /* contain.c */	EXTERN pset *sf_list ARGS((pset_family));
  /* contain.c */	EXTERN pset *sf_sort ARGS((pset_family, cmp_fn/*ESP_PFI*/));
  ...
  /* contain.c */	EXTERN pset_family sf_ind_contain ARGS((pset_family, int *));
  /* contain.c */	EXTERN pset_family sf_ind_unlist ARGS((pset *, int, int, int *, pset));
  /* contain.c */	EXTERN pset_family sf_merge ARGS((pset *, pset *, pset *, int, int));
  ...
  /* cvrin.c   */	EXTERN void PLA_labels ARGS((pPLA));
  /* cvrin.c   */	EXTERN char *get_word ARGS((FILE *, char *));
  /* cvrin.c   */	EXTERN int label_index ARGS((pPLA, char *, int *, int *));
  /* cvrin.c   */	EXTERN int read_pla ARGS((FILE *, esp_bool, esp_bool, int, pPLA *));
  ...
  /* cvrin.c   */	EXTERN void skip_line ARGS((FILE *, FILE *, esp_bool));
  /* cvrm.c    */	EXTERN int foreach_output_function ARGS((pPLA, ESP_PFI2, ESP_PFI1 ));
  /* cvrm.c    */	EXTERN int cubelist_partition ARGS((pcube *, pcube **, pcube **, unsigned int));
  /* cvrm.c    */	EXTERN int so_both_do_espresso ARGS((pPLA, int, int));
  /* cvrm.c    */	EXTERN int so_both_do_exact ARGS((pPLA, int, int));
  /* cvrm.c    */	EXTERN int so_both_save ARGS((pPLA, int));
  /* cvrm.c    */	EXTERN int so_do_espresso ARGS((pPLA, int, int));
  /* cvrm.c    */	EXTERN int so_do_exact ARGS((pPLA, int, int));
  ...
  /* cvrm.c    */	EXTERN pcover mini_sort ARGS((pcover, cmp_fn/*ESP_PFI*/));
  ...
  /* cvrm.c    */	EXTERN void so_both_espresso ARGS((pPLA, int, int));
  /* cvrm.c    */	EXTERN void so_espresso ARGS((pPLA, int, int));
  ...
  /* cvrout.c  */	EXTERN void makeup_labels ARGS((pPLA));
  /* cvrout.c  */	EXTERN void kiss_output ARGS((FILE *, pPLA));
  /* cvrout.c  */	EXTERN void kiss_print_cube ARGS((FILE *, pPLA, pcube, char *));
  /* cvrout.c  */	EXTERN void output_symbolic_constraints ARGS((FILE *, pPLA, int));
  ...
  /* equiv.c   */	EXTERN void  find_equiv_outputs ARGS((pPLA));
  /* equiv.c   */	EXTERN int check_equiv ARGS((pcover, pcover));
  /* espresso.c */	EXTERN pcover espresso ARGS((pcover, pcover, pcover, int));
  /* essen.c   */	EXTERN esp_bool essen_cube ARGS((pcover, pcover, pcube));
  ...
  /* exact.c   */	EXTERN pcover minimize_exact ARGS((pcover, pcover, pcover, int));
  /* exact.c   */	EXTERN pcover minimize_exact_literals ARGS((pcover, pcover, pcover, int));
  /* expand.c  */	EXTERN esp_bool feasibly_covered ARGS((pcover, pcube, pcube, pcube));
  /* expand.c  */	EXTERN int most_frequent ARGS((pcover, pcube));
  /* expand.c  */	EXTERN pcover all_primes ARGS((pcover, pcover));
  /* expand.c  */	EXTERN pcover esp_expand ARGS((pcover, pcover, esp_bool));
  ...
  /* gasp.c    */	EXTERN pcover last_gasp ARGS((pcover, pcover, pcover, cost_t *cost));
  /* gasp.c    */	EXTERN pcover super_gasp ARGS((pcover, pcover, pcover, cost_t *cost));
  ...
  /* hack.c    */	EXTERN void find_inputs ARGS((pcover, pPLA, symbolic_list_t *,
						     int, int, pcover *, pcover *));
  /* hack.c    */	EXTERN void form_bitvector ARGS((pset, int, int, symbolic_list_t *));
  /* hack.c    */	EXTERN void map_dcset ARGS((pPLA));
  /* hack.c    */	EXTERN void map_output_symbolic ARGS((pPLA));
  /* hack.c    */	EXTERN void map_symbolic ARGS((pPLA));
  /* hack.c    */	EXTERN pcover map_symbolic_cover ARGS((pcover, symbolic_list_t *, int));
  /* hack.c    */	EXTERN void symbolic_hack_labels ARGS((pPLA, symbolic_t*, pset, int, int, int));
  /* irred.c   */	EXTERN esp_bool cube_is_covered ARGS((pcube *, pcube));
  /* irred.c   */	EXTERN esp_bool taut_special_cases ARGS((pcube *));
  /* irred.c   */	EXTERN esp_bool tautology ARGS((pcube *));
  ...
  /* irred.c   */	EXTERN sm_matrix *irred_derive_table ARGS((pcover, pcover, pcover));
  /* map.c     */	EXTERN pset minterms ARGS((pcover));
  /* map.c     */	EXTERN void explode ARGS((int, int));
  /* map.c     */	EXTERN void map ARGS((pcover));
  /* opo.c     */	EXTERN void output_phase_setup ARGS((pPLA, int));
  /* opo.c     */	EXTERN pPLA set_phase ARGS((pPLA));
  /* opo.c     */	EXTERN pcover opo ARGS((pcube, pcover, pcover, pcover, int, int));
  /* opo.c     */	EXTERN pcube find_phase ARGS((pPLA, int, pcube, int));
  ...
  /* opo.c     */	EXTERN pset_family opo_recur ARGS((pcover, pcover, pcube, int, int, int, int));
  /* opo.c     */	EXTERN void opoall ARGS((pPLA, int, int, int));
  /* opo.c     */	EXTERN void phase_assignment ARGS((pPLA, int, int));
  /* opo.c     */	EXTERN void repeated_phase_assignment ARGS((pPLA, int));
  /* pair.c    */	EXTERN void generate_all_pairs ARGS((ppair, int, pset, int (*action)(register ppair pair)));
  ...
  /* pair.c    */	EXTERN void pair_free ARGS((ppair));
  /* pair.c    */	EXTERN void pair_all ARGS((pPLA, int));
  /* pair.c    */	EXTERN pcover delvar ARGS((pcover, esp_bool *));
  ...
  /* pair.c    */	EXTERN void print_pair ARGS((ppair));
  /* pair.c    */	EXTERN void find_optimal_pairing ARGS((pPLA, int));
  /* pair.c    */	EXTERN void set_pair ARGS((pPLA));
  /* pair.c    */	EXTERN void set_pair1 ARGS((pPLA, esp_bool));
  /* primes.c  */	EXTERN pcover primes_consensus ARGS((pcube *));
  /* reduce.c  */	EXTERN esp_bool sccc_special_cases ARGS((pcube *, pcube *));
  ...
  /* set.c     */	EXTERN esp_bool set_andp ARGS((pset, pset, pset));
  /* set.c     */	EXTERN esp_bool set_orp ARGS((pset, pset, pset));
  /* set.c     */	EXTERN esp_bool setp_disjoint ARGS((pset, pset));
  /* set.c     */	EXTERN esp_bool setp_empty ARGS((pset));
  /* set.c     */	EXTERN esp_bool setp_equal ARGS((pset, pset));
  /* set.c     */	EXTERN esp_bool setp_full ARGS((pset, int));
  /* set.c     */	EXTERN esp_bool setp_implies ARGS((pset, pset));
  ...
  /* set.c     */	EXTERN pset_family sf_copy_col ARGS((pset_family, int, pset_family, int));
  ...
  /* set.c     */	EXTERN pset_family sf_new_const ARGS((int));
  ...
  /* setc.c    */	EXTERN esp_bool ccommon ARGS((pcube, pcube, pcube));
  /* setc.c    */	EXTERN esp_bool cdist0 ARGS((pcube, pcube));
  /* setc.c    */	EXTERN esp_bool full_row ARGS((pcube, pcube));
  ...
  /* sminterf.c */	EXTERN pset do_sm_minimum_cover ARGS((pset_family));
  ...
  /* unate.c   */	EXTERN pset_family unate_intersect ARGS((pset_family, pset_family, esp_bool));
  /* verify.c  */	EXTERN void PLA_permute ARGS((pPLA, pPLA));
  /* verify.c  */	EXTERN esp_bool PLA_verify ARGS((pPLA, pPLA));
  /* verify.c  */	EXTERN esp_bool check_consistency ARGS((pPLA));
  /* verify.c  */	EXTERN esp_bool verify ARGS((pcover, pcover, pcover));
  /* globals.cc */	EXTERN void FIND_STATUS(esp_bool&, int, cost_t&, cost_t&);  
  /* minimize.cc */ EXTERN pcover minimize ARGS((pcover, pcover, int, int));

  /* Allocate and deallocate sets */
  inline pset set_new(int size) {  
    return set_clear(ALLOC(set_t, SET_SIZE(size)), size);
  }
  inline pset set_full(int size) {
    return set_fill(ALLOC(set_t, SET_SIZE(size)), size);
  }
  inline pset set_save(pset r) {
    return set_copy(ALLOC(set_t, SET_SIZE(NELEM(r))), r);
  }
  inline void set_free(pset r) {FREE(r);}
  /* Check for set membership, remove set element and insert set element */
  inline esp_bool is_in_set(pset set, int e) {
    return (set[WHICH_WORD(e)] & (1 << WHICH_BIT(e)));
  }
  inline esp_bool set_remove(pset set, int e) {
    return (set[WHICH_WORD(e)] &= ~ (1 << WHICH_BIT(e)));
  }
  inline esp_bool set_insert(pset set, int e) {
    return (set[WHICH_WORD(e)] |= 1 << WHICH_BIT(e));
  }
  ...
  typedef pset pcube;
  typedef pset_family pcover;
  inline pset new_cube() {return set_new(cube.size);}
  inline pcover new_cover(int i) {return sf_new(i, cube.size);}
  inline void free_cube(pcube r) {set_free(r);}
  inline void free_cover(pcover r) {sf_free(r);}
  inline void free_cubelist(pcube* T) {FREE(T[0]); FREE(T);}
  
  const int NOCOMP=0;
  const int SNOCOMP=1;
  const int DCSIMPLIFY=2;
  const int my_SIMPLIFY=3;
  const int ESPRESSO=3;

  const int OBJ_CUBE=0;
  const int OBJ_LITERAL=1;
  const int OBJ_SUPPORT=2;

  
}

#endif //_SIS_ESPRESSO_H_
